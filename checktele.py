...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            username = str(msg[2])
            ch = str(msg[1])
            await event.edit(f"I will try `{username}` on  `{ch}`!")

            @sedthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت التلقائي"))
            async def _(event):
                if "on" in isauto:
                    msg = await event.edit(f"التثبيت وصل لـ({trys})")
                elif "off" in isauto:
                    await event.edit("التثبيت غير شغال 😴!")
                else:
...
by @o_z_g
    ''')
                        ...
                        await sedthon.send_message(event.chat_id, f'''Erorr with  {username}
    Erorr :
    {str(eee)}''')
                        if "A wait of" in str(eee):
                            break
            ...
            isclaim.clear()
            isclaim.append("off")
            await sedthon.send_message(event.chat_id, "Done pro - @o_z_g")
...
async def _(event):
    if ispay2[0] == "yes":
        trys = 0
        msg = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
        if msg[0] == "خاصيه":  # تثبيت تلقائي عدد يوزر قناة
            isauto.clear()
            isauto.append("on")
            msg = ("".join(event.text.split(maxsplit=2)[2:])).split(" ", 2)
            username = str(msg[2])
